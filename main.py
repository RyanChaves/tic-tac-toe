...
def end_game():
    ...
    if winner_symbol == 'X':
        ...
        tie_label.grid(row=5, column=3)

    reset_btn.grid(row=5, column=1, columnspan=2)
    disable_btns()
...
bottom_label = Label(text='Tic-Tac-Toe', bg=YELLOW, font=(FONT_NAME, 20, "bold"))
bottom_label.grid(row=4,column=1, columnspan=3)
...
reset_btn = Button(root,text='Restart?',font=(FONT_NAME, 20, "bold"), command=reset_game)
